...
import {uploadFileOnS3} from '@functions/aws/storage';
...
const CameraRecord = ({navigation, challengeId}) => {
  ...
  const onMediaCaptured = useCallback(
    (media, type) => {
      ...
      console.log(`Media captured! ${JSON.stringify(type)}`);
      const file = {
        uri: media.path,
        name: media.path.split('/').pop(),
        type: 'video/mp4',
      };
      uploadFileOnS3(file, '.mp4', data => {
        navigation.navigate('ClashSubmitted', {
          challengeId,
